...
def calc_virus_scores(series, level, epitope_len):
    ...
    grouped = series[series].groupby(level=level)#chnaged by TK for compatibility
    #grouped = series.groupby(level=level)
    # seies has no attibute order
    for virus in nhits_per_virus[nhits_per_virus > 0].sort_values(ascending=False).index:
    #for virus in nhits_per_virus[nhits_per_virus > 0].index:
...
if __name__ == '__main__':
    ...
    hits[(beads_nhits > 2) | (samps_nhits < 2)] =False 
