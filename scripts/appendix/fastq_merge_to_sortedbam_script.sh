...
do
	...
	echo "bowtie2 --very-sensitive-local -x $genomeIndex/vir3 $merged_data/$samplename.fastq -p 8 -S $alignment/${samplename}.sam"| bsub -J ${samplename} -P virscan -e err.txt -o out.log -n 8
# -w is the dependency argument in bsub. 
	echo "samtools view -Sb $alignment/${samplename}.sam > $bamfiles/${samplename}.bam" | bsub -J ${samplename}_view -P virscan -e err.txt -o out.log -w "done('"${samplename}"')"
#change the -w to what you have named the previous -J such as -w "done('"${samplename}_view"')" and so .
#samtools sort
    echo "samtools sort $bamfiles/${samplename}.bam -o $bamsorted/${samplename}_A.sorted.bam" | bsub -J ${samplename}_sort -P virscan -e err.txt -o out.log -w "done('"${samplename}_view"')"
#samtools index
    echo "samtools index $bamsorted/${samplename}_A.sorted.bam" | bsub -J ${samplename}_index -P virscan -e err.txt -o out.log -w "done('"${samplename}_sort"')"
